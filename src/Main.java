...
public class Main {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        // Проверка на подключение базы
        // Если нет подключения то выведит ошибку
        // В противоположно случаем программа будет работать дальше
        connectDataBase(URL, userName, password); // функция для проверки подключения к базе
        while (true){
                ...
                case 4:
                    ...
                    try{
                        ...
                        if (newUserName.isEmpty() || newEmail.isEmpty() || NewPassword.isEmpty()) {
                            ...
                            updateUser(idUpdateUser, newUserName, newEmail, NewPassword);
                        }
                    }catch (NumberFormatException e){
    ...
    private static void deleteUser(int index)throws SQLException{
    ...
    private static boolean findUserById(int id)throws SQLException{
        final String CHECK_USER_UPDATE = "Select COUNT(*) from users where id = ?";
        try(Connection connection = DriverManager.getConnection(URL, userName, password);
            PreparedStatement preparedStatement = connection.prepareStatement(CHECK_USER_UPDATE)){
    ...
    private static void updateUser(int id, String newUsername, String NewUserEmail, String newUserPassword)throws SQLException{
    ...
    private static void insertRecordUser(User user)throws SQLException{
        ...
        try(Connection connection = DriverManager.getConnection(URL, userName, password)){
            PreparedStatement preparedStatementCheckToDublicate = connection.prepareStatement(CHECK_USER_SQL);
